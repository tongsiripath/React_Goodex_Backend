...
export const getNavLinkss = (req, res) => {
  const q = req.query.cat
    ? "SELECT * FROM tbl_navbarlinks WHERE cat=?"
    : "SELECT * FROM tbl_navbarlinks";
...
export const getNavLinks = (req, res) => {
  // const q =
...
export const addNavLinks = (req, res) => {
...
export const deleteNavLinks = (req, res) => {
  const token = req.cookies.access_token;
...
// export const updateNavLinks = (req, res) => {
//   const token = req.cookies.access_token;
//   if (!token) return res.status(401).json("Not authenticated!");
